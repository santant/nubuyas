<template>
  <div id="cart">
    ...
    <ul v-model="dataList" class="cartList">
      <li  v-for="(trollyDetail, indexs) in dataList">
        <div class="div_select" v-tap='{methods:updateCheck, index:indexs}' >
          <b :class="trollyDetail.isOK ? 'activeSelect':'' "><i class="icon iconfont">&#xe672;</i></b>
        </div>
        <div class="div_comenter" v-tap='{methods:updateCheck, index:indexs}' >
          <div class="img_cart">
          ...
          <ol class="img_msg">
            ...
            <li>
              <span v-if="undefined !== trollyDetail.sku && null !== trollyDetail.sku">{{ trollyDetail.sku | splitSku
                }}</span>
            ...
            <li>
              ...
              <span
                v-if="undefined !== trollyDetail.sku && null !== trollyDetail.sku">{{ trollyDetail.sku | splitSkuLast
                }}</span>
              <span v-else>{{ trollyDetail.worksSize }} mm</span>
            </li>
            <!-- <li>{{ trollyDetail.worksShape }}</li> -->
            <li>{{trollyDetail.createdDt}}</li>
            <li class="pic" v-model="trollyDetail.num">¥&nbsp;{{trollyDetail.price * trollyDetail.num | toFixedTwo}}
            </li>
          </ol>
        </div>
    ...
    <div class="cart_btn">
      <div class="all_select">
        <b :class="checkAllBtn == false ? '':'activeSelect' " v-tap='{methods:checkAll}'><i
          v-bind:hidden="checkAllBtn == false" class="icon iconfont" style="top: -0.95rem;">&#xe672;</i></b>
        <span>全选</span>
      </div>
      <div class="price">
        ...
        if (switchBool == true) {
          ...
          Api.car.submitCars(jsons).then(res => {
            if (res.data.code == 'success') {
              ...
              this.$router.push({
                path: '/payOrder',
                query: {'openId': res.data.openId, 'userDbId': localStorage.getItem("userDbId")}
              });
            } else {
              Toast('请求错误');
            }
